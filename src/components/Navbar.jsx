  ...
  return (
    <header className="sticky top-0 z-40 bg-transparent dark:bg-slate-950/20 bg-white/20 backdrop-blur-[3px] border-b dark:border-slate-800/40 border-slate-200/40 transition-all">
      <div className="w-full px-3 sm:px-6 lg:px-10">
        ...
        {/* ROW 1: Left Stats | CENTER BIG HEADING | Right: Theme Toggle -> Insights -> Saved */}
        <div className="grid grid-cols-3 items-center min-h-[72px] py-2 gap-2">
          
          {/* LEFT: Brand Emblem & Live Stats */}
          <div className="flex items-center gap-3 justify-self-start">
            <div 
              onClick={() => setViewMode('split')}
              className="relative flex items-center justify-center w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-emerald-500 via-teal-600 to-cyan-500 shadow-lg shadow-emerald-500/25 text-white font-black text-xl cursor-pointer hover:scale-105 transition-transform shrink-0"
            >
          ...
          <div 
            className="flex flex-col items-center justify-center justify-self-center text-center cursor-pointer select-none" 
            onClick={() => setViewMode('split')}
          >
          ...
          {/* RIGHT: Theme Toggle (FIRST) -> Insights -> Saved */}
          <div className="flex items-center gap-2 sm:gap-3 justify-self-end">
            
            {/* 1. THEME TOGGLE (Placed first before Insights) */}
            <ThemeToggle 
              isDark={isDark} 
